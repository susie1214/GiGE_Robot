...
void MainWindow::onDetect() {
    ...
    //        QMessageBox::warning(this,"Detect","ONNX load failed"); return;
    //    }
    //}
    //auto res = det_.infer(frame);
    //det_.draw(frame, res);
    //updateView(frame);
    if (!liveDetect_) {
...
void MainWindow::onUiTick() {
    //cv::Mat frame;
    //{ std::scoped_lock lk(frameMtx_); if (lastFrame_.empty()) return; frame = lastFrame_.clone(); }
